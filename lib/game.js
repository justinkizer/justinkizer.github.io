...
const init = () => {
  ...
  const tick = event => {
    ...
    for (var i = 0; i < customers.length; i++) {
      ...
      if (customers[i].x === bars[customers[i].barIndex].posXLimits[1] + 30) {
        ...
        createjs.Tween.get(animation, { loop: false, override: true})
          ...
          setTimeout(() => {stage.addChild(gameOver); clearInterval(customerGenerator);}, (bars[customers[i].barIndex].posXLimits[1] - bars[customers[i].barIndex].posXLimits[0]) * 5);
      }
      ...
      if (customers.length < 1) {
        ...
        setTimeout(() => stage.addChild(levelWon), 1000);
    ...
    customer.tweens.push(createjs.Tween.get(customer, { loop: false, override: true})
      ...
      if (originalx - 115 > bars[customer.barIndex].posXLimits[0] - 35) {
        setTimeout(() => {
          ...
          emptySteinTween = createjs.Tween.get(emptyStein, { loop: false })
          .to({ x: bars[customer.barIndex].posXLimits[1] + 40 }, ((bars[customer.barIndex].posXLimits[1] - bars[customer.barIndex].posXLimits[0]) * 10), createjs.Ease.getPowInOut(1)); //.to({ alpha: 0, x: bars[barIndex].posXLimits[0] - 25 }, 100)
        // createjs.Tween.get(customer0, { loop: false })
        //   .to({ x: bars[randomBarIndex].posXLimits[1] + 30 }, randomSpeed, createjs.Ease.getPowInOut(1));
        stage.addChild(emptyStein);}, 1000);
      }
  ...
  const createCustomers = () => {
    ...
    customer1.tweens = [];
    customer1.tweens.push(createjs.Tween.get(customer1, { loop: false })
      .to({ x: bars[randomBarIndex].posXLimits[1] + 30 }, randomSpeed, createjs.Ease.getPowInOut(1)));
    ...
    customer2.tweens = [];
    customer2.tweens.push(createjs.Tween.get(customer2, { loop: false })
      .to({ x: bars[randomBarIndex].posXLimits[1] + 30 }, randomSpeed, createjs.Ease.getPowInOut(1)));
  };
    ...
    const restartIdle = (e) => {
      if (e.keyCode === 37 || e.keyCode === 39) {
        ...
        steinTween = createjs.Tween.get(fullStein, { loop: false })
        ...
        idleTimeout = setTimeout(() => animation.gotoAndPlay("idle"), 300);
